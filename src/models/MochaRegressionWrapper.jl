module MochaRegressionWrapper

using Mocha
using Compat

export MWrap, forwardbackward, evaluateTestNN, initMochaBackend, evaluateTestNNPrediction
...
type MWrap
    ...
    # Constructor
    function MWrap(data_layer::MemoryDataLayer,
      barebonesNN::Function,
      name::AbstractString,
      bck::Backend,
      storage::Bool)

        (common_layers, storage_layer, loss_layer) = barebonesNN()

        if storage
            net = Net(name,bck,[data_layer,common_layers..., storage_layer])
        else
...
"""
Evaluate outputs (eg predictions) over whole dataset
"""
...
"""
Evaluate accuracy and loss over the whole dataset.
"""
...
"""
function getpredictions(mw::MWrap, para::Array{Float64,1}, batch_size::Int64)

   mw = setparams!(mw,para)
   Mocha.reset_statistics(mw.net)
   res = forward_epoch_outputs(mw.net)
